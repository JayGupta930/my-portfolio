...
const GAME_CONFIG = getGameConfig(GAME_IDS.HANGMAN);
...
const Hangman = ({ embedded = false }) => {
  ...
  useEffect(() => {
    if (gameStatus === 'won') {
      const finalScore = (maxWrongGuesses - wrongGuesses) * 15 + 10;
    ...
    <div className={containerClass}>
      <div className="flex flex-col w-full h-full gap-2 text-white">
        ...
        <div className="flex-1 flex flex-col items-center justify-center min-h-0 gap-3">
          ...
          {/* Keyboard */}
          <div className={`grid grid-cols-5 gap-1 ${embedded ? 'max-w-[260px]' : 'max-w-md'} w-full px-1`}>
            {alphabet.map((letter) => {
